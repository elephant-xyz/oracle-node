...
- Register the verified publication in the canonical catalog:
  ...
  The updater reads the public artifacts back and rejects a coverage/county or
  publication-scope mismatch. `full` requires a verified `county_total` denominator. Pilots use
  `published_subset`, so a complete pilot subset must never be described as full-county coverage.
  Missing or unsupported scope fails closed. Use `--permit-query-table-url` and
  `--places-table-url` when those artifacts are published.
- Include `catalog/published-counties.json` in the county publication PR. It is the
  authoritative enumeration consumed by Elephant MCP and Watchog. Never add a county before its
  public URLs have been read back and validated; publication is incomplete until the catalog PR
  is merged.
