...
Live probe (2026-09-03): window `2026-09-01`–`2026-09-03` returned Accela’s **`Showing 1-10 of 100`** cap, so a 3-day span is already truncated. Sample record `BC-RMR-26-00368` (parcel `022715274860000100`, Tarpon Springs). **48-hour countywide estimate fails** at that density (hundreds of thousands of details at ~5–10s each). Operator still asked to start the full Accela PINELLAS date-window harvest locally: job `pinellas-accela-full-20260903`, 1990-01-01 through today, concurrency 1, `--skip-existing`, heartbeat in `downloads/pinellas/permits/<jobId>/status.json`. City portals are not in this pass. Resume by re-running the same job id.
...
| Accela PINELLAS   | list cap 100; 3-day window already at cap (~33+/day lower bound) | 2026-09-03 probe: `Showing 1-10 of 100`; 1 detail `BC-RMR-26-00368`                         | **1** (Lee used 2–4; stay serial locally)                                  | **Above 48h** — operator started full local date-window anyway                  | **Bulk date-window harvest** in progress (`pinellas-accela-full-20260903`, 1990–today). Not 311k parcel search. Cities still runtime.                                                              |
