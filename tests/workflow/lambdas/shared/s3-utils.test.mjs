...
describe("s3-utils", () => {
  ...
  describe("uploadContentToS3", () => {
    it("should upload string content directly to S3", async () => {
      mockSend.mockResolvedValue({});

      const { uploadContentToS3 } = await import(
        "../../../../workflow/lambdas/shared/s3-utils.mjs"
      );
      ...
      const result = await uploadContentToS3(
        ...
        { bucket: "content-bucket", key: "content/string.txt" },
        mockLog,
      );

      expect(result).toBe("s3://content-bucket/content/string.txt");

      // Verify logging
      expect(mockLog).toHaveBeenCalledWith("info", "upload_s3_content", {
        bucket: "content-bucket",
        key: "content/string.txt",
      });

      // Verify S3 was called with correct content
      expect(mockSend).toHaveBeenCalledWith({
        input: expect.objectContaining({
    ...
    it("should upload Buffer content directly to S3", async () => {
      mockSend.mockResolvedValue({});
      ...
      const result = await uploadContentToS3(
        bufferContent,
        { bucket: "buffer-bucket", key: "buffer/data.bin" },
        mockLog,
      );

      expect(result).toBe("s3://buffer-bucket/buffer/data.bin");
    ...
    it("should upload content with custom content type", async () => {
      ...
      await uploadContentToS3(
        '{"data": "json"}',
        { bucket: "json-bucket", key: "api/response.json" },
        mockLog,
        "application/json",
      );
    ...
    it("should propagate S3 errors during content upload", async () => {
      mockSend.mockRejectedValue(new Error("Content upload failed"));
      ...
      ).rejects.toThrow("Content upload failed");
    });

    it("should upload empty string content", async () => {
      mockSend.mockResolvedValue({});
      ...
      const result = await uploadContentToS3(
        "",
        { bucket: "empty-bucket", key: "empty.txt" },
        mockLog,
      );

      expect(result).toBe("s3://empty-bucket/empty.txt");
    ...
    it("should upload large content", async () => {
      ...
      const result = await uploadContentToS3(
        largeContent,
        { bucket: "large-bucket", key: "large-file.txt" },
        mockLog,
      );

      expect(result).toBe("s3://large-bucket/large-file.txt");
