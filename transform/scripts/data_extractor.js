...
function extractProperty($) {
  ...
  sections.each((_, section) => {
    $(section)
      .find("table.appraisalAttributes")
      .each((_, table) => {
        $(table)
          .find("tr")
          .each((_, row) => {
            ...
            if (
              ...
              while (currentRow.length) {
                ...
                if (
                  ...
                  dataRows.length > 0 &&
                  !rowCells.toArray().some((cell) => cleanText($(cell).text()))
                ) {
                  break;
                }
              ...
              if (headerRow && dataRows.length) {
                ...
                if (descColIndex !== null && dataRows.length) {
                  ...
                  if (descColIndex < dataCells.length) {
                    rawUseCodeDescription = cleanText(
                      $(dataCells[descColIndex]).text(),
                    );
                    useCodeDescription = rawUseCodeDescription.toLowerCase();

                    console.log(
                      `Use Code Description: "${rawUseCodeDescription}"`,
                    );
                    console.log(
                      `Use Code Description (lowercase): "${useCodeDescription}"`,
                    );
  ...
  function tryMapPropertyType(typeText, rawValue) {
    ...
    if (
      lowerText.includes("commercial, vacant") ||
    ...
    // Mobile Home variations
      ...
      lowerText.includes("mobile home subdivision") ||
    ...
    // Interval Ownership/Time Share
      ...
      lowerText.includes("interval ownership") ||
    ...
    // General keyword matching (fallback for non-Lee County data)
      ...
      ["single family", "single-family"].some((keyword) =>
        lowerText.includes(keyword),
      )
    ) {
      matched = "SingleFamily";
    } else if (
      lowerText.includes("duplex") ||
      lowerText.includes("2 unit") ||
      lowerText.includes("two unit")
    ) {
      matched = "2Units";
    } else if (
      lowerText.includes("triplex") ||
      lowerText.includes("3 unit") ||
      lowerText.includes("three unit")
    ) {
      matched = "3Units";
    } else if (
      lowerText.includes("fourplex") ||
      lowerText.includes("4 unit") ||
      lowerText.includes("four unit")
    ) {
      matched = "4Units";
    } else if (
      ["townhouse", "town house", "townhome"].some((keyword) =>
        lowerText.includes(keyword),
      )
    ) {
      matched = "Townhouse";
    } else if (
      lowerText.includes("condominium") ||
      lowerText.includes("condo")
    ) {
      if (lowerText.includes("detached")) {
        matched = "DetachedCondominium";
      } else if (
        lowerText.includes("non warrantable") ||
        lowerText.includes("nonwarrantable")
      ) {
      ...
      lowerText.includes("cooperative") ||
      lowerText.includes("co-op")
    ) {
      matched = "Cooperative";
    } else if (
      ...
      if (
        ["multi", "double", "triple", "wide"].some((keyword) =>
          lowerText.includes(keyword),
        )
      ) {
      ...
      ["pud", "planned unit", "planned development"].some((keyword) =>
        lowerText.includes(keyword),
      )
    ) {
      matched = "Pud";
    } else if (
      lowerText.includes("timeshare") ||
      lowerText.includes("time share")
    ) {
      matched = "Timeshare";
    }
  ...
  if (!matchedType && useCodeDescription) {
    [matchedType, rawSourceValue] = tryMapPropertyType(
      useCodeDescription,
      rawUseCodeDescription,
    );
  }
  ...
  if (!propertyType) {
    $("div.sectionSubTitle").each((_, title) => {
      ...
      if (rawText.includes("condominium")) {
        ...
        rawText.includes("single family") ||
        rawText.includes("single-family")
      ) {
...
function extractAddress($, unAddr) {
  ...
  if (line1) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (
        (preDirectional &&
          directionalMappings[partUpper] === preDirectional &&
          !streetNameParts.some(
            (p) => directionalMappings[p.toUpperCase()] === preDirectional,
          )) ||
        i === mainSuffixIdx ||
        (postDirectional &&
          directionalMappings[partUpper] === postDirectional &&
          mainSuffixIdx !== null &&
          i > mainSuffixIdx)
      ) {
        continue;
