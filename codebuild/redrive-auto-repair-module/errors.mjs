 ...
 * @param {string} params.tableName - DynamoDB table name.
 * @param {DynamoDBDocumentClient} [params.documentClient] - Optional document client (uses default if not provided).
 * @returns {Promise<string[]>} - Resolves with array of error hashes from deleted execution-error links.
 */
export async function deleteExecution({
  ...
  do {
    ...
    if (response.Items && response.Items.length > 0) {
      for (const item of response.Items) {
        const pk = /** @type {string} */ (item.PK);
        const sk = /** @type {string} */ (item.SK);
        itemsToDelete.push({
          PK: pk,
          SK: sk,
        });
        
        // Collect error hashes from execution-error links
        // Error links have SK format: ERROR#{errorHash}
        // Extract hash from SK if this is an error link (not the execution root)
        if (sk.startsWith("ERROR#")) {
          const hash = sk.replace(/^ERROR#/, "");
          if (hash) {
  ...
  for (const batch of batches) {
    ...
    while (remaining.length > 0) {
      ...
      if (unprocessed.length > 0) {
        console.warn(
          `⚠️ ${unprocessed.length} item(s) were not processed in batch delete for execution ${executionId}. Will retry...`,
        );
      }
      ...
      attempts += 1;
      if (attempts > BATCH_WRITE_MAX_RETRIES) {
        const errorMsg = `Exceeded BatchWrite retry attempts while deleting execution ${executionId}. ${unprocessed.length} item(s) remain unprocessed.`;
        console.error(`❌ ${errorMsg}`);
        console.error(`Unprocessed items:`, JSON.stringify(unprocessed, null, 2));
  ...
  // Verify deletion by querying again (for debugging)
  ...
  if (verifyResponse.Items && verifyResponse.Items.length > 0) {
    console.warn(
      `⚠️ WARNING: Deletion verification failed for execution ${executionId}. ${verifyResponse.Items.length} item(s) still exist. This might indicate items are being recreated.`,
    );
  } else {
    console.log(`✓ Verified: All items deleted for execution ${executionId}`);
  }
  
  return Array.from(errorHashes);
}

/**
 * Delete error aggregates that are no longer referenced by any execution-error links.
 * Checks if any execution-error links still reference each error hash before deleting the aggregate.
 *
 * @param {object} params - Deletion parameters.
 * @param {string[]} params.errorHashes - Array of error hashes to check and potentially delete.
 * @param {string} params.tableName - DynamoDB table name.
 * @param {DynamoDBDocumentClient} [params.documentClient] - Optional document client (uses default if not provided).
 * @returns {Promise<number>} - Number of aggregates deleted.
 */
export async function deleteOrphanedErrorAggregates({
  ...
  console.log(
    `Checking ${errorHashes.length} error aggregate(s) for orphaned references...`,
  );

  for (const errorHash of errorHashes) {
    const errorPk = buildErrorPk(errorHash);
    
    // Check if any execution-error links still reference this error hash
    const queryParams = {
      TableName: tableName,
      IndexName: "ErrorHashExecutionIndex",
      KeyConditionExpression: "GS1PK = :errorPk",
    ...
    // If no execution-error links reference this error hash, delete the aggregate
    if (!response.Items || response.Items.length === 0) {
      try {
        await client.send(
          new BatchWriteCommand({
        ...
        deletedCount++;
        console.log(`✓ Deleted orphaned error aggregate: ${errorHash}`);
      } catch (deleteError) {
        console.error(
          `Failed to delete error aggregate ${errorHash}:`,
          deleteError instanceof Error ? deleteError.message : String(deleteError),
        );
      }
    } else {
      console.log(
        `Skipping error aggregate ${errorHash}: still referenced by ${response.Items.length} execution-error link(s)`,
      );
    }
  }

  console.log(
    `Deleted ${deletedCount} orphaned error aggregate(s) out of ${errorHashes.length} checked`,
  );
