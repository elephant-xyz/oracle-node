...
async function populateProxies(tableName, proxyFilePath) {
    ...
    for (const line of lines) {
        ...
        if (!isValidProxyFormat(proxyUrl)) {
            console.warn(`[WARN] Invalid proxy format (expected username:password@ip:port): ${proxyUrl}`);
            continue;
        }
        ...
        try {
            ...
            console.error(`[ERROR] Failed to add proxy ${maskProxyUrl(proxyUrl)}: ${errorMsg}`);
    ...
    if (proxyCount === 0) {
        ...
        console.log(`[INFO] Successfully populated ${proxyCount} proxies into DynamoDB table`);
    }
