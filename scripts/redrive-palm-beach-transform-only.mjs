#!/usr/bin/env node
/**
 * redrive-palm-beach-transform-only.mjs
 *
 * Checkpointed transform-only redrive driver for the Palm Beach County
 * palm-beach-property-first-seed-all-20260630 job.
 *
 ...
 *     node scripts/redrive-palm-beach-transform-only.mjs --dry-run
 *
 *   # Live test — 20 parcels
 *   AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
 *     node scripts/redrive-palm-beach-transform-only.mjs --limit 20
 *
 *   # Full run (detached, log to .redrive-logs/)
 *   nohup env AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
 *     node scripts/redrive-palm-beach-transform-only.mjs \
...
const DEFAULT_JOB_ID = "palm-beach-property-first-seed-all-20260630";
...
const DEFAULT_OUTPUTS_PREFIX = `outputs/palm-beach-property-first-seed/${DEFAULT_JOB_ID}`;
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \\
    node scripts/redrive-palm-beach-transform-only.mjs [options]
...
function parseArgs(argv) {
  ...
  return {
    bucket,
    jobId,
    outputsPrefix: `outputs/palm-beach-property-first-seed/${jobId}`,
...
const SCHEMA_VERSION = "oracle-node.palm-beach-transform-only-redrive.v1";
