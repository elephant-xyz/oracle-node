 ...
 * @property {string} sourceCsvS3Uri - Source Lee county seed CSV S3 URI.
 ...
 * @property {boolean} skipExistingNeon - Skip rows whose Lee Appraiser property is already present in Neon.
 ...
 * @property {Set<string>} requestIdentifiers - Existing Lee Appraiser request/Folio identifiers.
 * @property {Set<string>} normalizedParcelIdentifiers - Existing normalized Lee Appraiser parcel identifiers.
 */
...
const DEFAULT_SOURCE_CSV_S3_URI = "s3://counties-seeds/lee.csv";
const DEFAULT_ENV_FILE = "../elephant-query-db/.env.local";
const DEFAULT_JOB_ID = `lee-property-first-seed-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}`;
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/enqueue-lee-appraisal-property-first-from-seed.mjs \
      ...
      --job-id lee-property-first-seed-YYYYMMDD \
      --limit 0 --send-delay-ms 50

Options:
  --source-csv-s3-uri <s3uri>       Full Lee seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
  ...
  --workflow-output-base-uri <s3uri>
                                    Appraisal workflow output base. Default: environment bucket / outputs / lee-property-first-seed / job id.
  --property-first-permit-output-prefix <s3uri>
                                    Permit artifact prefix. Default: environment bucket / permit-harvest / lee-property-first-seed.
...
async function resolveOptions(cli) {
  ...
  return {
    ...
    generatedSeedPrefix:
      ...
      `s3://${environmentBucketName}/seed-inputs/lee-property-first-seed/${cli.jobId}`,
    workflowOutputBaseUri:
      cli.workflowOutputBaseUri ??
      `s3://${environmentBucketName}/outputs/lee-property-first-seed/${cli.jobId}`,
    propertyFirstPermitOutputPrefix:
      cli.propertyFirstPermitOutputPrefix ??
      `s3://${environmentBucketName}/permit-harvest/lee-property-first-seed`,
  };
}
 ...
 * @returns {string | undefined} Uppercase alphanumeric parcel identifier.
 */
...
/**
 * Read existing Lee Appraiser identifiers from Neon so the S3 seed file can be
 * used to queue only new properties instead of replaying already-loaded rows.
 *
...
async function readExistingNeonIdentifiers(cli) {
  ...
  const pool = new Pool({
    ...
    application_name: "lee-appraisal-seed-enqueue",
 ...
 *
 * @param {SeedRow} row - Parsed source seed row.
 * @param {ExistingNeonIdentifiers} existing - Existing Lee Appraiser identifiers from Neon.
...
async function uploadSeedAndEnqueueWorkflow({
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "lee_appraisal_property_first_workflow_enqueued",
      enqueuedIndex,
      sourceRowNumber,
 ...
 * @param {ExistingNeonIdentifiers} existing - Existing Lee Appraiser identifiers used for duplicate skips.
 * @returns {Promise<void>} Resolves when the requested rows have been processed.
 */
async function enqueueFromSeed(options, existing) {
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "lee_appraisal_property_first_seed_enqueue_complete",
...
async function main() {
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "lee_appraisal_property_first_seed_enqueue_start",
  ...
  await enqueueFromSeed(options, existing);
}
