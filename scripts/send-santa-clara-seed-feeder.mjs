...
/**
 * Santa Clara property-first seed-feeder sender.
 *
 * Builds and sends ONE Santa Clara feeder message to the permit-harvest queue.
 * The feeder is the self-requeuing full-run trigger: it drips the whole Santa
 * Clara seed CSV through the appraisal -> permit pipeline with backpressure,
 ...
 * (-> `elephant-oracle-node-prepare-queue-santa-clara`), not from this message.
 */
const COUNTY = {
  /** Feeder message discriminator routed by the permit-harvest worker. */
  feederType: "santa-clara-property-first-seed-feeder",
   ...
   */
  prepareQueueName: "elephant-oracle-node-prepare-queue-santa-clara",
};
...
function suggestedJobId() {
  ...
  return `santa-clara-property-first-seed-all-${date}`;
}
 ...
 * @property {string} sourceCsvS3Uri - Santa Clara seed CSV S3 URI.
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/send-santa-clara-seed-feeder.mjs --job-id <id> [--dry-run]
...
Options:
  ...
  --source-csv-s3-uri <s3uri>
                          Santa Clara seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
...
requeues, dripping the whole Santa Clara seed through the pipeline with workflow
...
/**
 * Build the Santa Clara property-first seed-feeder message from resolved infra.
 *
...
async function main() {
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "santa_clara_property_first_seed_feeder_sent",
      jobId: cli.jobId,
      feederQueueUrl,
