/**
 * Persistent run state, failure ledger, and progress helpers for Hillsborough local ingest.
...
/**
 * Classify an error message for retry policy.
 * Transient errors are safe to auto-retry; permanent ones are not.
 * @param {unknown} error
 ...
 * @param {string} outputRoot
 * @param {string} jobId
 * @param {Partial<RunProgress>} seed
 * @returns {Promise<RunProgress>}
 */
...
/**
 * Rolling window tracker for instantaneous throughput (e.g. 60-second window).
...
export async function writeRunProgress(outputRoot, jobId, progress, meta = {}) {
  ...
  // Only measure active downloaded/transformed (succeeded + failed) for rolling throughput
  // to avoid skew when rapidly skipping thousands of already completed files on startup
  ...
  if (rollingSamples.length >= 2) {
    ...
    if (deltaMin > 0 && deltaDone >= 0) {
      rate = deltaDone / deltaMin;
    }
  } else {
...
/**
 * Load unique failure folios (latest attempt wins). Transient/unknown are retryable.
 * @param {string} outputRoot
 * @param {string} jobId
 ...
 *   maxAttempts?: number;
 *   baseDelayMs?: number;
 *   onRetry?: (info: { attempt: number; error: unknown; classification: FailureClass }) => void;
 * }} [options]
 * @returns {Promise<T>}
 */
