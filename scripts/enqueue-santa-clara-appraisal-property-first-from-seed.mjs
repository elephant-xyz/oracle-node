...
/**
 * Santa Clara county configuration. This file is a parameterized clone of
 * scripts/enqueue-orange-appraisal-property-first-from-seed.mjs; only the values
 * below (and the parcel-identifier normalizer) differ from the Orange run.
 *
 ...
 * this object. With a seed `county` value of "Santa Clara" the state machine
 * targets `elephant-oracle-node-prepare-queue-santa-clara`.
 */
const COUNTY = {
 ...
 * @property {string} sourceCsvS3Uri - Source Santa Clara county seed CSV S3 URI.
 ...
 * @property {boolean} skipExistingNeon - Skip rows whose Santa Clara Appraiser property is already present in Neon.
 ...
 * @property {Set<string>} requestIdentifiers - Existing Santa Clara Appraiser request/APN identifiers.
 * @property {Set<string>} normalizedParcelIdentifiers - Existing normalized Santa Clara Appraiser parcel identifiers.
 */
...
const DEFAULT_SOURCE_CSV_S3_URI = "s3://counties-seeds/santa_clara.csv";
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/enqueue-santa-clara-appraisal-property-first-from-seed.mjs \
      --source-csv-s3-uri s3://counties-seeds/santa_clara.csv \
      --job-id ${COUNTY.slug}-YYYYMMDD \
      --limit 0 --send-delay-ms 50

Options:
  --source-csv-s3-uri <s3uri>       Full Santa Clara seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
...
/**
 * Normalize a Santa Clara parcel identifier (APN) for duplicate checks and S3
 * key names.
 *
 * Santa Clara APNs are 8-digit numeric strings (e.g. `21350014`); the seed's
 * `parcel_id`/`source_identifier` carry the bare 8-digit value and the SC crawler
 * queries sccgov with that same value (`{"apn":["21350014"]}`). Only non-digit
 * characters are stripped — the result is NOT zero-padded — so this matches the
 * canonical value the loader writes to Neon, whose `normalizeParcelIdentifier`
 * (query-db-loader/normalizers.js) is digits-only with NO padding. This is the
 * SAME digits-only-no-pad rule Palm Beach uses; it differs from Orange, which
 * zero-pads to 15 for its 15-digit PID format. Zero-padding an 8-digit APN to 15
 * would produce `000000021350014`, which never matches the loader's `21350014`,
 * so skipExistingNeon dedup would fail and every parcel would be re-enqueued.
 * Separator/whitespace differences normalize away; every digit is preserved.
 *
 * @param {unknown} value - Raw parcel identifier.
 * @returns {string | undefined} Digits-only parcel identifier (no zero-pad).
 */
...
/**
 * Read existing Santa Clara Appraiser identifiers from Neon so the S3 seed file
 ...
 *
 * @param {SeedRow} row - Parsed source seed row.
 * @param {ExistingNeonIdentifiers} existing - Existing Santa Clara Appraiser identifiers from Neon.
 ...
 * @param {ExistingNeonIdentifiers} existing - Existing Santa Clara Appraiser identifiers used for duplicate skips.
