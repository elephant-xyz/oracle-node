...
/**
 * Broward property-first seed-feeder sender.
 *
 * Builds and sends ONE Broward feeder message to the permit-harvest queue.
 * The feeder is the self-requeuing full-run trigger: it drips the whole Broward
 ...
 * (-> `elephant-oracle-node-prepare-queue-broward`), not from this message.
 */
const COUNTY = {
  /** Feeder message discriminator routed by the permit-harvest worker. */
  feederType: "broward-property-first-seed-feeder",
   ...
   */
  prepareQueueName: "elephant-oracle-node-prepare-queue-broward",
};
...
function suggestedJobId() {
  ...
  return `broward-property-first-seed-all-${date}`;
}

// The worker re-streams the county seed from row 1 on each wakeup to skip to
// the checkpoint. Use a large batch so that scan is amortized over many
// enqueues per wakeup instead of being repaid every few hundred rows.
 ...
 * @property {string} sourceCsvS3Uri - Broward seed CSV S3 URI.
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/send-broward-seed-feeder.mjs --job-id <id> [--dry-run]
...
Options:
  ...
  --source-csv-s3-uri <s3uri>
                          Broward seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
...
requeues, dripping the whole Broward seed through the pipeline with workflow
...
/**
 * Build the Broward property-first seed-feeder message from resolved infra.
 *
...
function buildFeederMessage({
  ...
  return {
    ...
    // worker role lacks sqs:GetQueueAttributes on the Broward prepare queue
    ...
    // prepare entry once the role is granted access to the Broward queue.
    backpressureQueues: [
      {
        name: "workflow",
...
async function main() {
  ...
  const [
    ...
    // NOTE: the Broward prepare queue URL is intentionally NOT resolved here —
    // prepare-
    // queue backpressure is omitted because the permit-harvest worker role lacks
    // sqs:GetQueueAttributes on the per-county Broward prepare queue (see the omission
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "broward_property_first_seed_feeder_sent",
      jobId: cli.jobId,
      feederQueueUrl,
