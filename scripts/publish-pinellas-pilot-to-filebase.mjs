...
export function parseCliOptions(argv) {
  ...
  return {
    ...
    envFile: values.get("env-file") ?? path.join(DEFAULT_QUERY_DB_DIR, ".env.local"),
...
/**
 * Upload parquet + coverage through the elephant-query-db Filebase publishers.
 *
 ...
 * @returns {Promise<void>} Resolves after both uploads (or dry-run).
 */
export async function publishPinellasArtifactsToFilebase({
  ...
  const queryDbDir = path.resolve(repoRoot, options.queryDbDir);
  ...
  process.env.FILEBASE_QUERY_TABLE_IPNS_LABEL ??= `oracle-query-table-${COUNTY}`;
  if (
  ...
  process.env.FILEBASE_COVERAGE_IPNS_LABEL ??= `oracle-dataset-coverage-${COUNTY}`;

  if (!hasFilebaseCredentials(process.env)) {
    throw new Error(
      `Filebase credentials are missing. Create ${envFile} with S3_ACCESS_KEY_ID, S3_SECRET_ACCESS_KEY, and FILEBASE_API_TOKEN (or access+secret so the token can be derived).`,
    );
  }

  const tsx = path.join(queryDbDir, "node_modules", ".bin", "tsx");
  const publishArgs = options.dryRun ? ["--dry-run"] : [];
  await runCommand(
    tsx,
    [
      "scripts/upload-query-table-to-filebase.ts",
      "--county",
      COUNTY,
      "--parquet",
      parquetPath,
  ...
  await runCommand(
    tsx,
    [
      "scripts/upload-coverage-to-filebase.ts",
      "--county",
      COUNTY,
      "--coverage",
      coveragePath,
