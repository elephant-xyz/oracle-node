...
/**
 * Broward county configuration. This file is a parameterized clone of
 ...
 * this object. With a seed `county` value of "Broward" the state machine
 * targets `elephant-oracle-node-prepare-queue-broward`.
 */
const COUNTY = {
 ...
 * @property {string} sourceCsvS3Uri - Source Broward county seed CSV S3 URI.
 ...
 * @property {boolean} skipExistingNeon - Skip rows whose Broward Appraiser property is already present in Neon.
 ...
 * @property {Set<string>} requestIdentifiers - Existing Broward Appraiser request/Folio identifiers.
 * @property {Set<string>} normalizedParcelIdentifiers - Existing normalized Broward Appraiser parcel identifiers.
 */
...
const DEFAULT_SOURCE_CSV_S3_URI = "s3://counties-seeds/broward.csv";
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/enqueue-broward-appraisal-property-first-from-seed.mjs \
      --source-csv-s3-uri s3://counties-seeds/broward.csv \
      --job-id ${COUNTY.slug}-YYYYMMDD \
      --limit 0 --send-delay-ms 50

Options:
  --source-csv-s3-uri <s3uri>       Full Broward seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
...
then the state machine enqueues the property-first permit worker to fetch
permits and load appraisal+permits to Neon in one transaction. Broward folios
are alphanumeric (condos include letters) and must not be digit-stripped.
`);
}
...
/**
 * Normalize a Broward folio for duplicate checks and S3 key names.
 *
 * Keep the folio as text. Condos include letters, so stripping non-digits or
 * padding numeric values would change parcel identity and silently omit them.
 *
 * @param {unknown} value - Raw parcel identifier.
 * @returns {string | undefined} API-ready folio.
 */
function normalizeParcelIdentifier(value) {
  return normalizeBrowardFolio(value);
}
...
/**
 * Read existing Broward Appraiser identifiers from Neon so the S3 seed file
 ...
 *
 * @param {SeedRow} row - Parsed source seed row.
 * @param {ExistingNeonIdentifiers} existing - Existing Broward Appraiser identifiers from Neon.
 ...
 * @param {ExistingNeonIdentifiers} existing - Existing Broward Appraiser identifiers used for duplicate skips.
