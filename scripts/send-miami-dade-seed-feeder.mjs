...
/**
 * Miami-Dade property-first seed-feeder sender.
 *
 * Builds and sends ONE Miami-Dade feeder message to the permit-harvest queue.
 * The feeder is the self-requeuing full-run trigger: it drips the whole Miami-
 * Dade seed CSV through the appraisal -> permit pipeline with backpressure,
 ...
 * (-> `elephant-oracle-node-prepare-queue-miami-dade`), not from this message.
 */
const COUNTY = {
  /** Feeder message discriminator routed by the permit-harvest worker. */
  feederType: "miami-dade-property-first-seed-feeder",
   ...
   */
  prepareQueueName: "elephant-oracle-node-prepare-queue-miami-dade",
};
...
function suggestedJobId() {
  ...
  return `miami-dade-property-first-seed-all-${date}`;
}

// Seed CSV is ~929 MiB / 933,532 rows and the worker re-streams it from row 1
// each wakeup to skip to the checkpoint. Use a LARGE batch so that expensive
// scan is amortized over many enqueues per wakeup (not re-paid per 200 rows).
// 2000 rows of seed-upload + workflow-enqueue fits comfortably inside the
// 900s/4GB worker.
 ...
 * @property {string} sourceCsvS3Uri - Miami-Dade seed CSV S3 URI.
...
Usage:
  AWS_PROFILE=elephant-oracle-node AWS_REGION=us-east-1 \
    node scripts/send-miami-dade-seed-feeder.mjs --job-id <id> [--dry-run]
...
Options:
  ...
  --source-csv-s3-uri <s3uri>
                          Miami-Dade seed CSV. Default: ${DEFAULT_SOURCE_CSV_S3_URI}
...
requeues, dripping the whole Miami-Dade seed through the pipeline with workflow
...
/**
 * Build the Miami-Dade property-first seed-feeder message from resolved infra.
 *
...
function buildFeederMessage({
  ...
  return {
    ...
    // worker role lacks sqs:GetQueueAttributes on the per-county prepare queue
    ...
    // prepare entry once the role is granted GetQueueAttributes on the queue.
    backpressureQueues: [
      {
        name: "workflow",
...
async function main() {
  ...
  const [
    ...
    // NOTE: the prepare queue URL is intentionally NOT resolved here — prepare-
    // queue backpressure is omitted because the permit-harvest worker role lacks
    // sqs:GetQueueAttributes on the per-county prepare queue (see the omission
  ...
  console.log(
    JSON.stringify({
      level: "info",
      message: "miami_dade_property_first_seed_feeder_sent",
      jobId: cli.jobId,
      feederQueueUrl,
