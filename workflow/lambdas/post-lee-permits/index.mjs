...
function resolveTransformLocation({ countyName, transformPrefixUri }) {
  ...
  // Force lee-permits transform zip regardless of seed county
  const transformKey = `${normalizedPrefix}/lee-permits.zip`;
...
export const handler = async (event) => {
  ...
  /** @type {string | null} */
  let preparedUri = (event && event.prepare && event.prepare.output_s3_uri) || null;
  /** @type {string | null} */
  let seedZipS3 = (event && event.seed_output_s3_uri) || null;
  /** @type {{ bucket: { name: string }, object: { key: string } } | null} */
  const s3Msg = event && event.s3 ? event.s3 : null;

  if (!preparedUri || !seedZipS3) {
    if (!s3Msg || !s3Msg.bucket || !s3Msg.bucket.name || !s3Msg.object || !s3Msg.object.key) {
      throw new Error(
        "Either provide prepare.output_s3_uri and seed_output_s3_uri or a valid s3 { bucket.name, object.key } message",
      );
    }
    const bucketName = s3Msg.bucket.name;
    const key = s3Msg.object.key;
    // Expect keys like outputs/<runId>/output.zip
    const runPrefix = key.replace(/\/output\.zip$/, "");
    preparedUri = `s3://${bucketName}/${runPrefix}/output.zip`;
    seedZipS3 = `s3://${bucketName}/${runPrefix}/seed_output.zip`;
  }
  ...
  try {
    ...
    const { scriptsZipPath } =
      await transformScriptsManager.ensureScriptsForCounty({
        // Use a fixed cache key for lee-permits scripts
        countyName: "lee-permits",
        transformBucket,
        transformKey,
        log,
      });
