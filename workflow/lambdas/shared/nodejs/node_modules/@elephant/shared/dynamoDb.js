 ...
 * @property {string} pk - Partition key in format ERROR#<countyName>#<hash>
 * @property {string} sk - Sort key in format ERROR#<countyName>#<hash>
...
/**
 * Queries all error entries with the same error hash for a specific county
 * @function queryErrorsByHash
 * @param {string} tableName - DynamoDB table name
 * @param {string} errorMessage - The error message to search for
 * @param {string} countyName - The county name to search for
 * @returns {Promise<ErrorEntry[]>} Array of error entries with the same hash for the county
 * @throws {Error} If the query fails
 * @example
 * import { queryErrorsByHash } from '@elephant/shared';
 * const errors = await queryErrorsByHash('ErrorsTable', 'Validation failed', 'Miami-Dade');
 */
export async function queryErrorsByHash(tableName, errorMessage, countyName) {
    const errorHash = createErrorHash(errorMessage);
 ...
 * const errorType = await upsertErrorType('ErrorsTable', 'Validation failed', 'Miami-Dade');
 */
export async function upsertErrorType(tableName, errorMessage, countyName, additionalAttributes = {}) {
    const errorHash = createErrorHash(errorMessage);
    const timestamp = new Date().toISOString();
 ...
 * const lockedErrorType = await acquireLock('ErrorsTable', 'Validation failed', 'Miami-Dade');
 */
export async function acquireLock(tableName, errorMessage, countyName) {
    const errorHash = createErrorHash(errorMessage);
 ...
 * const resolvedErrorType = await releaseLockAndResolve('ErrorsTable', 'Validation failed', 'Miami-Dade');
 */
export async function releaseLockAndResolve(tableName, errorMessage, countyName) {
    const errorHash = createErrorHash(errorMessage);
...
/**
 * Finds all records with the same error hash that are ready for reprocessing
 * @function findRecordsForReprocessing
 * @param {string} tableName - DynamoDB table name
 * @param {string} errorMessage - The error message to find records for
 * @param {string} countyName - The county name for the error
 * @returns {Promise<ErrorEntry[]>} Array of error entries that need reprocessing
 * @throws {Error} If the scan fails
 * @example
 * import { findRecordsForReprocessing } from '@elephant/shared/dynamoDb';
 * const records = await findRecordsForReprocessing('ErrorsTable', 'Validation failed', 'Miami-Dade');
 */
export async function findRecordsForReprocessing(tableName, errorMessage, countyName) {
    const errorHash = createErrorHash(errorMessage);

    // First get all records with this error hash
    ...
    try {
        ...
        // Filter out the ErrorType entry (which has sk = ERROR#countyName#hash) and return only ErrorEntry records
        return queryResponse.Items.filter(item => !item.sk.startsWith('ERROR#'));
    } catch (error) {
        throw new Error(`Failed to find records for reprocessing: ${error.message}`);
    }
}
