...
export function createTransformScriptsManager({ s3Client, persistentCacheRoot }) {
  ...
  return {
    /**
     * @param {EnsureScriptsParams} params
     * @returns {Promise<EnsureScriptsResult>}
     */
    async ensureScriptsForCounty({ countyName, transformBucket, transformKey, log }) {
      const normalizedCountyName = countyName.toLowerCase();
      await ensureDirectory(persistentCacheRoot);
      const countyCacheDir = path.join(persistentCacheRoot, normalizedCountyName);
      await ensureDirectory(countyCacheDir);
      const localZipPath = path.join(countyCacheDir, `${normalizedCountyName}.zip`);
      ...
      try {
        ...
        if (!remoteMd5) throw new Error("HeadObject returned empty ETag");
      } catch (error) {
        throw new Error(
      ...
      const validCache = await validateLocalCache(normalizedCountyName, remoteMd5, localZipPath);
      if (validCache) return validCache;

      log("info", "download_transform_zip", { transform_bucket: transformBucket, transform_key: transformKey });
      const remoteObject = await s3Client.send(new GetObjectCommand({ Bucket: transformBucket, Key: transformKey }));
      const remoteBytes = await remoteObject.Body?.transformToByteArray();
      if (!remoteBytes) throw new Error(`Failed to download county transform from S3 for ${countyName}`);
