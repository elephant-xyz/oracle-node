...
// Return first row from any CSV in CWD as a simple object
function readFirstCsvRow() {
  const files = fs.readdirSync('.').filter(f => f.toLowerCase().endsWith('.csv'));
...
// Extract company name from Licensed Professional block
function extractCompany($) {
  let name = null;
  const $lp = $('#tbl_licensedps');
  if ($lp.length) {
    const html = $lp.find('td').eq(1).html() || '';
    const lines = html
      .split(/<br\s*\/?>/i)
  ...
  if (!name) {
    const $label = $('*:contains("Licensed Professional")').filter((i, el) => /Licensed Professional/i.test($(el).text()));
    if ($label.length) {
      const $next = $label.first().closest('tr,div,section').nextAll('table').first();
      if ($next.length) {
        const html = $next.find('td').eq(1).html() || '';
        const lines = html
          .split(/<br\s*\/?>/i)
          .map(s => s.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim())
          .filter(Boolean);
  ...
  if (!name) {
    const $alt = $('*:contains("Contractor"):not(:has(*)), *:contains("Company"):not(:has(*))').first();
    if ($alt.length) {
      const t = $alt.parent().find('td,span,div').eq(1).text().trim() || $alt.next().text().trim();
      if (t) name = t;
    }
  }
...
function normalizePhone(s) {
  if (!s) return null;
  const m = (typeof s === 'string' ? s : String(s)).match(/\+?1?[\s.\-]?\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}/);
  if (!m) return null;
...
// Extract communication (email, phone) from Licensed Professional block, fallback to whole doc
function extractCommunication($) {
  ...
  if ($lp.length) {
    ...
    const lines = html
      ...
      .map(s => s.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim())
      .filter(Boolean);
    const pLine = lines.find(l => /phone/i.test(l)) || lines.find(l => /\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}/.test(l));
    phone = normalizePhone(pLine);
    const eLine = lines.find(l => /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.test(l));
    if (eLine) email = (eLine.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i) || [null])[0];
  }
  if (!phone) {
    const t = $.text();
    const m = t.match(/\+?\d?[\s.\-]?\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}/);
  ...
  return { email_address: email || null, phone_number: phone || null };
}
...
// Also emit company and communication objects and their relationship when available
try {
  const htmlContent = fs.readFileSync(htmlFile, 'utf8');
  const $ = cheerio.load(htmlContent);
  const company = extractCompany($);
  const comm = extractCommunication($);
  const hasCompany = company && company.name && company.name.trim();
  const hasComm = comm && (comm.email_address || comm.phone_number);

  // use the first improvement’s request info, or a fallback
  const baseRequest = improvements[0]?.source_http_request || {
    method: 'GET',
    url: 'https://www.leegov.com/dcd/BldPermitServ',
  ...
  if (hasCompany) {
    const companyObj = {
      name: company.name,
      request_identifier: `company_${propertyInfo.parcelId}`,
      source_http_request: baseRequest
    };
    fs.writeFileSync(path.join(dataDir, 'company.json'), JSON.stringify(companyObj, null, 2));
    console.log('✓ company.json created');
  }

  if (hasComm) {
    const commObj = {
      email_address: comm.email_address || null,
      phone_number: comm.phone_number || null
    };
    fs.writeFileSync(path.join(dataDir, 'communication.json'), JSON.stringify(commObj, null, 2));
    console.log('✓ communication.json created');
  }

  if (hasCompany && hasComm) {
    const cc = { from: { '/': './company.json' }, to: { '/': './communication.json' } };
    fs.writeFileSync(path.join(dataDir, 'company_to_communication.json'), JSON.stringify(cc, null, 2));
    console.log('✓ company_to_communication.json created');
  }

  // optional: emit property_improvement_to_company.json using external ID from CSV
  try {
    const csvRow = readFirstCsvRow();
    const externalId = csvRow && (csvRow.id || csvRow.ID || csvRow.improvement_id || csvRow.cid || csvRow.CID);
    if (hasCompany && externalId) {
      const rel = { from: { '/': String(externalId).trim() }, to: { '/': './company.json' } };
      fs.writeFileSync(path.join(dataDir, 'property_improvement_to_company.json'), JSON.stringify(rel, null, 2));
      console.log('✓ property_improvement_to_company.json created');
    }
  } catch (err) {
    console.warn('Could not emit property_improvement_to_company.json:', err.message);
  }
} catch (err) {
  console.error('Error extracting company/communication:', err.message);
}
